...
- ※上記の理解は必要か?
```
CFI directiveは通常、高度なデバッグ等のためにコンパイラが自動生成するものなので、後々は必要かもしれないが、今は深く理解する必要はないと思われる
```
...
- スタックとリターンアドレス
```
・呼び出した関数が終了した際に、もともとの実行時に戻ってくる必要がある
・そのため単なるJumpとは異なる
・もともと実行していたアドレスをリターンアドレスと呼ぶ
・関数呼び出しは再帰的になるケースがあるので、リターンアドレスはメモリ上のスタックに保存される

・つまり、mainからfunc_aを読んで、func_aからfunc_bを呼ぶ場合
　func_bからreturnで、func_aの呼び出し元まで戻るわけだが、この際にリターンアドレスを利用しているのか。
```
- 関数呼び出し時の詳細な動作
```
★
1. 現在のプログラムカウンタの値=次に実行される命令のアドレスをスタックにpushする
2. サブルーチンの先頭アドレスへjump。
3. サブルーチンからの復帰時にはスタックしたリターンアドレスをpopすることで、呼び出し元へ復帰）
・サブルーチンへ実行を移すことがサブルーチン呼び出し(call)
・サブルーチンを終了し元の処理へ実行を移すことが(return)
```
- その他(スタックポインタでスタックが表現できるわけ)
```

・スタックはスタックの一番上のアドレスを保持する1つの変数のみを使って実装可能。(これは、スタックがメモリ内の連続したアドレス範囲を使用しているため)
・pushすると一般的にスタックポインタの値は1つ小さくなり、popで1つ大きくなる。
・スタックのトップを保持している記憶領域をスタックポインタと呼ぶ

・x86-64は関数を使ったプログラミングのサポート用に、スタックポインタ専用のレジスタと、そのレジスタを利用する命令をサポートしている
・最初にスタックの場所を指定すればスタックポインタは自動で更新される。
...
```
.globl main, plus
    この2つの関数はグローバルなのでプログラム全体から見える

add
    ・x86-64の整数演算命令は通常2つのレジスタしか受け取らない
    ・第1引数のレジスタを上書きする形で結果は保存される
call plus
    ・callの次の命令。つまり、retのアドレスをstackにpush
    ・これはプログラムカウンタが指し示すのが次の命令アドレスだから
    ・callの引数として与えられたアドレスにJump
    ・つまり、plus関数の先頭アドレスにJumpする

mov rax, rsi
    movはunixでいうcpコマンド。データは移動でなくcopyされる
    つまり、rdi, 3も代入されていると考えればいい
...
mainのret
    plusの返り値がraxに入った状態なので、
    これをそのままmainの返り値にできる
    つまり、プログラムとしては7が返り値になるので終了コードは7となりエラー扱いとなる
    ※echo $? -> 7
...
```
https://godbolt.org/
・Cをアセンブリにした結果を表示してくれる
・.globlや.inital_syntaxは表示されていないようなので注意
・またaddでなく、lea eax, [rdi+rsi]になっていた省略されている？
```
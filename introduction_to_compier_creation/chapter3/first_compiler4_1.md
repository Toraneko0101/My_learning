...
※()の場合、一度primaryまでいって、そこからexprを呼び
結果がprimaryに返り、そこからまたexprまで返ってくる
```
- 呼び出し関係の図
```
1         +       2       *       3
...
- 再帰下降構文解析
```
・1つの生成規則を1つの関数にマップすること
・上記のparserではトークンを1つだけ先読みし、度の関数を呼び出すか、またはreturnするか決めた
・このようにトークンを1つだけ先読みする再帰下降パーサのことを、LL(1)パーサと呼ぶ。
・またLL(1)パーサが書ける文法のことをLL(1)文法と呼ぶ
```
